...
/**
 * Classe de acesso aos dados de Curso
 *
...
public class CursoDAO extends JpaDao<Curso> implements Serializable {

	private static final long serialVersionUID = -1L;

	private static Logger log = Logger.getLogger(CursoDAO.class);

	/**
	 * Salva ou altera cadasto o curso correspondente. Salva se o mesmo já contenha
	...
	public void salvarCurso(Curso curso) throws AcademicoException {
		...
		if (curso == null) {
			throw new AcademicoException("O Curso deve estar inicializado para inserçao ou alteraçao.");
		} else {
	 ...
	 * @throws AcademicoException
	 */
	public Curso pesquisarCursoPorCodigo(Long numero) throws AcademicoException {
	 ...
	 * @author  Marco Figueiredo
	 * @since   22/08/2017
	 * @return  {@link List<Curso>}
	 * @throws  AcademicoException
	 */
	@SuppressWarnings("unchecked")
	public List<Curso> listarTodosCursos() throws AcademicoException {
		try {
			return getEntityManager().createNamedQuery("Curso.findAll").getResultList();
		} catch (Exception ex) {
			log.error(ex);
